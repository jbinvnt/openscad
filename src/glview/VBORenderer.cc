...
void VBORenderer::add_shader_attributes(VertexArray& vertex_array,
                                        ...
                                        bool mirror, bool marked) const
{
  if (!shader_attributes_index) return;

  shared_ptr<VertexData> vertex_data = vertex_array.data();

  if (points.size() == 3 && getShader().data.csg_rendering.barycentric && getShader().data.csg_rendering.marked) {
    // Get edge states
...
void VBORenderer::add_shader_data(VertexArray& vertex_array)
{
  ...
  vertex_data->addAttributeData(std::make_shared<AttributeData<GLubyte, 1, GL_UNSIGNED_BYTE>>()); // marked
}
  ...
  if (getShader().data.csg_rendering.marked) {
    ...
    ss->glBegin().emplace_back([index, count, type, stride, offset, ss_ptr = std::weak_ptr<VertexState>(ss)]() {
      ...
      if (ss) {
        ...
        GL_CHECKD(glVertexAttribIPointer(index, count, type, stride, (GLvoid *)(ss->drawOffset() + offset)));
      }
    });
  }
...
void VBORenderer::shader_attribs_enable() const
{
  ...
  GL_CHECKD(glEnableVertexAttribArray(getShader().data.csg_rendering.marked));
}
void VBORenderer::shader_attribs_disable() const
{
  ...
  GL_CHECKD(glDisableVertexAttribArray(getShader().data.csg_rendering.marked));
}
